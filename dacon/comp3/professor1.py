...
X_data = np.loadtxt('./DATA/dacon/KAERI/train_features.csv',skiprows=1,delimiter=',')
X_data = X_data[:,1:]
print(X_data.shape)
     
    
Y_data = np.loadtxt('./DATA/dacon/KAERI/train_target.csv',skiprows=1,delimiter=',')
Y_data = Y_data[:,1:]
print(Y_data.shape)
...
X_data_test = np.loadtxt('./DATA/dacon/KAERI/test_features.csv',skiprows=1,delimiter=',')
...
def my_loss_E1(y_true, y_pred):
    return K.mean(K.square(y_true-y_pred)*weight1)/8e+04
...
def train(model,X,Y):
    MODEL_SAVE_FOLDER_PATH = './DACON/comp3/model/comp3'
...
submit = pd.read_csv('./DATA/dacon/KAERI/sample_submission.csv')
...
submit.to_csv('./DACON/comp3/submit/pro_submit_16.csv', index = False)
