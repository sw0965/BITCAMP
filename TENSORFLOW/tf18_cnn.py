...
x_img = tf.reshape(x, [-1, 28, 28, 1])  # conv2d 를 위한 리쉐잎


# w1 = tf.get_variable("w1", shape=[784,512], dtype=tf.float32,           # get_variable 초기값 알아서 설정, 메모리에서 유리
#                      initializer=tf.contrib.layers.xavier_initializer())
w1 = tf.get_variable("w1", shape=[3, 3, 1, 32]) # conv2d(32, ksize=(3, 3), input=(28,28), 1 = color) 
                     # 3, 3 = ksize, 1 = color, 32 = output
print("=============w1=================") # 텐서형이기 때문에 자료형만 나옴
print("w1 : ", w1)      # shape=(3, 3, 1, 32) 
'''
conv2d bias 계산이 된다고 하심 따로 해도 되는데 명시 안해줘도 됌
'''
...
with tf.compat.v1.Session() as sess: # Session을 close하지 않으려고
    ...
    _, acc = sess.run([hypothesis,accuracy], feed_dict={x_img: x_test, y: y_test, keep_prob : 0.6})
