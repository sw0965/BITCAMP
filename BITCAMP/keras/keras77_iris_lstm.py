...
#데이터 전처리 2. 정규화
x_train = x_train.reshape(120, 2, 2).astype('float32')/255
x_test = x_test.reshape(30, 2, 2).astype('float32')/255
...
model = Sequential()
model.add(LSTM(5, activation='relu', input_shape=(2, 2)))
...
hist          = model.fit(x_train, y_train, validation_split=0.5, epochs=1000, batch_size=1, verbose=1, callbacks=[es,cp,tb])
