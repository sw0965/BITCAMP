...
for thresh in thresholds:  # 칼럼 수 만큼 돈다!
    ...
    # print(type(y_train))
    selection_model = XGBClassifier(objective='multi:softmax', n_estimators=5, n_jobs=-1)

    selection_model.fit(select_x_train, y_train, verbose=True, eval_metric=['merror','mlogloss'], eval_set=[(select_x_train, y_train),(select_x_test, y_test)],
