...
#데이터 전처리 2. 정규화
x_train = x_train.reshape(309, 5, 2, 1).astype('float32')/255
x_test = x_test.reshape(133, 5, 2, 1).astype('float32')/255
...
model = Sequential()
model.add(Conv2D(5, (2, 2), padding='same', activation='relu', input_shape=(5, 2, 1)))
model.add(Flatten())
model.add(Dense(8))
model.add(Dense(16))   
model.add(Dense(8))   
