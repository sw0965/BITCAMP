...
#데이터 전처리 2. 정규화
x_train = x_train.reshape(398, 5, 2, 3).astype('float32')/255
x_test = x_test.reshape(171, 5, 2, 3).astype('float32')/255
...
model = Sequential()
model.add(Conv2D(5,(2,2),padding='same',activation='relu', input_shape=(5, 2, 3)))
model.add(Flatten())
model.add(Dense(4))
model.add(Dense(8))
model.add(Dense(16))   
