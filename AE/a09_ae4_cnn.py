...
def autoencoder(middle, hidden):
    ...
    model.add(Conv2D(filters=hidden, kernel_size=2, strides=(2,2), activation='relu', input_shape=(28, 28, 1), padding='valid'))
    model.add(Conv2D(filters=middle, kernel_size=2, strides=(2,2), activation='relu', padding='valid'))
    # model.add(Flatten())
    # model.add(Dense(64, activation='relu'))
    # model.add(Dense(7*7*64, activation='relu'))
    # model.add(Reshape(target_shape=(7,7,64)))
    model.add(Conv2DTranspose(filters=32, kernel_size=2, strides=(2,2), padding='valid', activation='relu'))
